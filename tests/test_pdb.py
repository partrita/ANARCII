...
def anarcii_model(tmp_path):
    ...
    model.number("data/raw_data/1kb5.pdb", str(tmp_path / "1kb5_anarcii.pdb"))

    model.number("data/raw_data/8kdm.pdb", str(tmp_path / "8kdm_anarcii.pdb"))
...
def test_files_are_identical(anarcii_model, filename):
    ...
    assert content1 == content2, f"Files {expected_file} and {test_file} are different!"

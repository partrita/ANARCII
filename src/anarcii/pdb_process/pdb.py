        ...
        with open(file_path, 'r') as pdb_file:
            for line in pdb_file:
                if line[:6].strip() in ATOM_RECORDS:
                    ...
                    if not sequences[chain_id] or sequences[chain_id][-1][0] != residue_key:
        ...
        for chain_id, sequence_info in sequences.items():
            ...
            if chain_call in ["H", "L", "K", "A", "B", "D", "G"]:
                ...
                if score < 19.0 and not \
                    ...
                    print("PDB chain: ", chain_id, "Failed, missing conserved residues. Score: ", score)
                    ...
                    print("PDB chain: ", chain_id, "ANARCII Chain (Score): ",
                        chain_call, f"({score})", " CHECK FOR TCR!")                    
                    print(len(sequence), sequence)
                
                else:
                    print("PDB chain: ", chain_id, "ANARCII Chain (Score): ", chain_call, f"({score})")
                ...
                for i in range(len(sequence)):
                    ...
                    if current_line in seen_lines:
                        ...
                        if i <= i2:
                            renumbering_scheme[sequence_info[i][0]] = (i + transform, ' ')                            
                        elif i3 < len(nums):

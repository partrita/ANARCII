...
def build_inward_list(length, start_num, end_num):
    cdrs = [x for x in range(27,38)] + [x for x in range(56,65)] + [x for x in range(105,117)]
    ...
    if int(start_num) in cdrs:
        ...
        for i in range(midpoint):
            result.append(
                (int(start_num), alphabet[i % len(alphabet)])
            )
...
def format_output(indices, names, numbering, alignment):
    ...
    output = [(index, nm, number, align)
              for index, nm, number, align in
              zip(indices, names, numbering, alignment)]

...
class Loader:
    ...
    def _load_params(self):
        ...
        param_path = pkg_resources.files(
            'anarcii.models').joinpath(self.type, param_filename)
    ...
    def _get_model_path(self):
        ...
        model_path = pkg_resources.files(
            'anarcii.models').joinpath(self.type, model_filename)
    ...
    def _load_model(self):
        ENC = model.Encoder(self.INPUT_DIM, self.HID_DIM, self.ENC_LAYERS,
                            self.ENC_HEADS, self.ENC_PF_DIM, self.ENC_DROPOUT,
                            self.device)

        DEC = model.Decoder(self.OUTPUT_DIM, self.HID_DIM, self.DEC_LAYERS,
                            self.DEC_HEADS, self.DEC_PF_DIM, self.DEC_DROPOUT,
                            self.device)

        S2S = model.S2S(ENC, DEC,
                        self.SRC_PAD_IDX,
                        self.TRG_PAD_IDX,
                        self.device)
